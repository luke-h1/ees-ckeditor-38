...
import { Editor, EditorConfig } from 'explore-education-statistics-ckeditor';
...
const useCKEditorConfig = ({
  ...
  const config = useMemo<EditorConfig>(() => {
    return {
      ...
      // type error here because of customUploadAdapterPlugin
      extraPlugins:
        ...
        ? {
            ...
            commentRemoved(markerId) {
              if (editorInstance?.current) {
                ...
                onAutoSave?.(editorInstance?.current.data.get());
              }
            },
        ...
        ? {
            async save() {
              if (editorInstance?.current) {
                ...
                await onAutoSave(editorInstance.current.data.get());
              }
            },
            waitingTime: 5000,
          }
